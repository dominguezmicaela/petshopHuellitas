...
const Row = ({ label, value, highlight }) => (
  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
    <span style={{ fontSize: "0.88rem", color: C.muted2, fontWeight: 600 }}>{label}</span>
    <span style={{ fontSize: "0.95rem", fontWeight: 700, color: highlight ? C.verde3 : C.verde2 }}>
      {value}
    </span>
  </div>
);
...
const CarritoResumen = ({ subtotal, envio, total }) => (
  <aside style={{
    ...
    <div style={{
      ...
      <h3 style={{ fontWeight: 800, fontSize: "1rem", color: C.sobreVerde, margin: 0 }}>Resumen de compra</h3>
    </div>
    ...
    {subtotal < 5000 && (
      <div style={{ marginBottom: "1.4rem" }}>
        <div style={{
          ...
          <p style={{ fontSize: "0.78rem", color: C.verde3, fontWeight: 700, margin: 0 }}>
             Te faltan ${(5000 - subtotal).toLocaleString("es-AR")} para envío gratis
          </p>
        </div>
        <div style={{ height: "6px", background: C.beige2, borderRadius: "99px", overflow: "hidden" }}>
          <div style={{
            height: "100%", borderRadius: "99px",
            width: `${Math.min((subtotal / 5000) * 100, 100)}%`,
            background: `linear-gradient(90deg, ${C.verde3}, #5a8a69)`,
    ...
    <div style={{ height: "1px", background: C.beige2, margin: "1.2rem 0" }}/>

    {/* Total */}
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.6rem" }}>
      <span style={{ fontWeight: 800, fontSize: "1rem", color: C.verde2 }}>Total</span>
      <span style={{ fontWeight: 800, fontSize: "1.5rem", color: C.verde2 }}>
    ...
    <Link to="/" style={{
      ...
      color: C.verde3, textDecoration: "none",
    ...
    <p style={{ textAlign: "center", fontSize: "0.75rem", color: C.muted, fontWeight: 600, marginTop: "1rem" }}>
